...
|---|---:|---:|---|---|---|
| AtlasHybridTestPlugin | 0.1.0-alpha | 1.19.2 | ✅ FULL | Constructor-safe logger, lifecycle, commands and tab completion, real async pre-login allow/deny, online-player registry/lookup, player events, block-break cancellation, scheduler, permission defaults/attachments/provider/services, stable player identity and diagnostics | Internal integration fixture |
...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Eleven official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/event APIs, real async pre-login, failed-enable rollback, vanilla Material, safe data-version metadata and real online-mode state now present | Raw boot #11 passes `AsyncPlayerPreLoginEvent` registration and stops at `PlayerLoginEvent` (`CORE_API`); normal stop exits cleanly, while nine daemon LuckPerms workers observed before stop and the known later permission injection remain external/architectural concerns; not bundled |
