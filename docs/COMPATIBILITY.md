...
|---|---:|---:|---|---|---|
| AtlasHybridTestPlugin | 0.1.0-alpha | 1.19.2 | ✅ FULL | Constructor-safe logger, lifecycle, commands and tab completion, player events, block-break cancellation, scheduler, permission defaults/attachments/provider/services, stable player identity and diagnostics | Internal integration fixture |
...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Four official raw boots; Permission Core, constructor-safe `JavaPlugin` context and command tab completion now present | Raw boot #4 passes `TabExecutor` and stops at the simple public overload `YamlConfiguration#loadConfiguration(File)` (`TRIVIAL`); later permission injection still assumes CraftBukkit internals; not bundled |
