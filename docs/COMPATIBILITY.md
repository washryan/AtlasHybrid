...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Twelve official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/event APIs, real async pre-login and synchronous login gate, failed-enable rollback, vanilla Material, safe data-version metadata and real online-mode state now present | Raw boot #12 resolves/registers the connection listener including `PlayerLoginEvent`, then stops at `ServerCommandEvent` (`CORE_API`) while registering the platform listener; `onEnable` does not complete, known later permissible injection remains architectural, and no external JAR is bundled |
