...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Five official raw boots; Permission Core, constructor-safe `JavaPlugin`, command completion and nested configuration loading now present | Raw boot #5 completes load and reaches enable, then stops at `Server#getOnlinePlayers()` (`CORE_API`); later permission injection still assumes CraftBukkit internals; not bundled |
