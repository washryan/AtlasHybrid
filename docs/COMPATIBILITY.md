...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Seven official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player APIs, explicit event registration and failed-enable rollback now present | Raw boot #7 passes `EventExecutor` registration and stops at `org.bukkit.Material` (`CORE_API`); rollback leaves no diagnosed live threads and normal stop exits; later permission injection still assumes CraftBukkit internals; not bundled |
