...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Seventeen official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/entity/event APIs, real login gates, local/RCON/player command events, plugin lifecycle events, failed-enable rollback, vanilla Material, safe data-version metadata, real online-mode state and thread-safe real game-mode context now present | Raw boot #17 crosses `GameMode`, completes command registration, then stops at public `org.bukkit.World$Environment` (`CORE_API`) while creating the context manager; LuckPerms `onEnable` does not complete, known later permissible injection remains architectural, and no external JAR is bundled |
