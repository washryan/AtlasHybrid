...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Fifteen official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/event APIs, real login gates, local/RCON/player command events, plugin lifecycle events, failed-enable rollback, vanilla Material, safe data-version metadata and real online-mode state now present | Raw boot #15 completes `BukkitConnectionListener`, `BukkitPlatformListener`, and `registerPlatformListeners`, initializes H2 storage, then stops at public `org.bukkit.entity.Entity` (`CORE_API`) while registering `BukkitCommandExecutor`; LuckPerms `onEnable` does not complete, known later permissible injection remains architectural, and no external JAR is bundled |
