...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED — ARCHITECTURAL DECISION | Twenty official raw boots; context manager, listeners, commands and H2 complete before the platform-hook boundary | Source audit confirms the Bukkit backend replaces private `SimplePluginManager` maps and later `CraftHumanEntity#perm`; the public LuckPerms API is sufficient for an Atlas `PermissionProvider` but is published only after those hooks. ADR-009 recommends an isolated public-API adapter contingent on a supported LuckPerms bootstrap contract and rejects fake CraftBukkit shapes, reflection and instrumentation |
