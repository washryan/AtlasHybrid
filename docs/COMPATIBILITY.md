...
| LuckPerms Forge bridges | 5.4.46 | Forge / tested on 1.19.2 | 🟡 PARTIAL / PASS | Optional public-API bridge makes LuckPerms TRUE/FALSE authoritative for Bukkit Player hasPermission; the same API instance is available through Bukkit ServicesManager; live contexts, reload and lifecycle proven | Permission query and public service discovery only. No Bukkit plugin identity/depend, Bukkit PlayerAdapter, Vault or subscription parity |
