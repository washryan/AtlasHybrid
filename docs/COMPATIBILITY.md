...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Fourteen official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/event APIs, real login gates, local/RCON/player command events, failed-enable rollback, vanilla Material, safe data-version metadata and real online-mode state now present | Raw boot #14 passes `PlayerCommandPreprocessEvent`, then stops at `PluginEnableEvent` (`CORE_API`) while reflecting the platform listener; `registerPlatformListeners` and `onEnable` do not complete, known later permissible injection remains architectural, and no external JAR is bundled |
