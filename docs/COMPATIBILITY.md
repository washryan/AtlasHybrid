...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Ten official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/event APIs, failed-enable rollback, vanilla Material, safe data-version metadata and real online-mode state now present | Raw boot #10 passes `Server#getOnlineMode()` and stops at `AsyncPlayerPreLoginEvent` (`CORE_API`); no external thread survived this run and the launcher exited normally, while the known later permission injection still assumes CraftBukkit internals; not bundled |
