...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Eighteen official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/entity/event APIs, real login gates, local/RCON/player command events, plugin lifecycle events, failed-enable rollback, vanilla Material, safe data-version metadata, real online-mode/game-mode/world-environment context now present | Raw boot #18 crosses `World.Environment`, completes command registration, then stops at public `org.bukkit.event.player.PlayerChangedWorldEvent` (`CORE_API`) while registering the context calculator; LuckPerms `onEnable` does not complete, known later permissible injection remains architectural, and no external JAR is bundled |
