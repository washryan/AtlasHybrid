...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED — ARCHITECTURAL DECISION | Twenty official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/entity/event APIs, real login gates, local/RCON/player command events, plugin lifecycle events, failed-enable rollback, vanilla Material, safe data-version metadata, real online-mode/game-mode/world context and both context-change invalidations now present | Raw boot #20 completes the context manager with `PlayerGameModeChangeEvent`, keeps command registration and H2 healthy, then stops in `setupPlatformHooks` at `InjectorSubscriptionMap` requiring `org.bukkit.plugin.SimplePluginManager`; LuckPerms `onEnable` does not complete and no CraftBukkit compatibility shape or external JAR is bundled |
