...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Nineteen official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/entity/event APIs, real login gates, local/RCON/player command events, plugin lifecycle events, failed-enable rollback, vanilla Material, safe data-version metadata, real online-mode/game-mode/world context and world-change invalidation now present | Raw boot #19 crosses `PlayerChangedWorldEvent`, keeps command registration and H2 healthy, then stops at public `org.bukkit.event.player.PlayerGameModeChangeEvent` (`CORE_API`) in the same context calculator; LuckPerms `onEnable` does not complete, known later permissible injection remains architectural, and no external JAR is bundled |
