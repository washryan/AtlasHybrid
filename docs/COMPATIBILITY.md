...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Six official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration APIs and session-backed online players now present | Raw boot #6 passes `Server#getOnlinePlayers()` and stops at `EventExecutor` (`CORE_API`); later permission injection still assumes CraftBukkit internals; not bundled |
