...
| LuckPerms | 5.5.81 | Bukkit / tested on 1.19.2 | ⛔ BLOCKED | Nine official raw boots; Permission Core, constructor-safe `JavaPlugin`, command/configuration/player/event APIs, failed-enable rollback, vanilla Material and safe data-version metadata now present | Raw boot #9 passes Adventure's `UnsafeValues#getDataVersion()` initialization and stops at `Server#getOnlineMode()` (`CORE_API`); failed partial initialization leaves two LuckPerms OkHttp threads alive after Minecraft stops; later permission injection still assumes CraftBukkit internals; not bundled |
